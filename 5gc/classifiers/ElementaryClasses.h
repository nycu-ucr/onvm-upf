...
struct Rule
{
    ...
    /* ---------- per-field data ----- */
    std::vector<unsigned>              prefix_length;   // IP masks etc.
    std::vector<std::array<Point, 2>>  range;           // [low, high]

    /* ---------- helpers ------------ */
    bool MatchesPacket(const Packet& p) const
    {
        for (int i = 0; i < dim; ++i)
