...
// 	cmap_node(const Rule& r) 
//     : priority(r.priority)
//     , desc(r.descriptor)
//     , rule_ptr(std::make_shared<Rule>(r))
//     , next(nullptr){ }
// 	unsigned int key;
// 	int priority;
//     uintptr_t desc;

//     //A std::shared_ptr<Rule> allocates a separate
//     // reference-count block and increments/decrements it on every lookup.
    
// 	//std::shared_ptr<Rule> rule_ptr;
    
//     // non-owning pointer to a heap copy
//     //const Rule *rule_ptr;
//     std::shared_ptr<Rule> rule_ptr; 
...
struct cmap_node {
    cmap_node(unsigned key) : key(key), next(nullptr) {}
    cmap_node(const Rule& r)
        : priority(r.priority),
          rule_ptr(std::make_shared<Rule>(r)),
          desc(r.descriptor),
