...
public:
	...
	MatchResult FindMatchPacketMod(const Packet& p) const;
	...
	cmap_node* InsertionReturnNode(const Rule& r);
...
public:
	...
	bool DeleteRuleByDescriptor(uintptr_t desc);
	MatchResult ClassifyAPacketMod(const Packet& one_packet) const;
...
protected:
	...
	// std::unordered_map<key,Tuple> of tuples, and inside each Tuple a linked list of cmap_node
	// O(1) average-time removal without touching the hot lookup path
	std::unordered_map<uintptr_t, std::pair<Tuple*, cmap_node*>> desc_map;
};
