...
/* Per-session buffering drain configuration */
...
packet_handler(struct rte_mbuf *pkt, struct onvm_pkt_meta *meta, struct onvm_nf_local_ctx *nf_local_ctx) {
    ...
    if (is_dl) {
        /* ── DL: always-enqueue into per-session ring ───────── */
        ...
        /* Encap (GTP-U outer header) — same for FORW and BUFF */
        if (far->flags.forwardingParameters &&
        ...
        /* QoS policing — FORW only; BUFF packets bypass QoS */
         ...
         * Bump refcnt so the framework's rte_pktmbuf_free (for the DROP
         * action we set below) only decrements 2→1 — the ring still
        ...
        /* Tell framework to free *this* ref (refcnt 2→1) */
        meta->action = ONVM_NF_ACTION_DROP;

        /* INLINE drain: immediate small batch for non-BUFF sessions */
        ...
        /* TICK drain: periodic scan when enqueue counter hits threshold */
        if (g_enqueue_counter >= TICK_THRESHOLD) {
            for (int si = 0; si < SESS_BUF_MAX_USERS; si++) {
                if (g_sess_buf[si].touched && !g_sess_buf[si].is_buffering)
                    drain_session_batch(si, TICK_DRAIN_BATCH, nf);
            }
            g_enqueue_counter = 0;
        }
