...
**BUFF mode**: DL only, via per-bucket per-family **override pipes** interleaved with the base DL classifier (`DL_SDF_BUFF_OVERRIDE_Px` / `DL_BUFF_OVERRIDE_Px`). The buffer slot is registered **before** the override is installed (register-first — no onset drop window; on override-add failure a fresh registration is undone via `dpu_buffer_rollback_register`, gated on `register_flow`'s return: 0 = newly registered, 1 = already ACTIVE). `update_far(BUFF)` *adds* an override entry (match = `rec->cached_match` verbatim, action `pkt_meta=htonl(rule_id)`, fwd → `TO_DPU_ARM_DL`); the base `DL_*_MATCH` entry is never touched. `update_far(FORW)` refreshes encap (`update_dlencap_only` — no-op if params unchanged; 2nd+ change auto-falls back to encap delete+add, see constraint 9) → `begin_drain` → `wait_drain_done` → *removes* the override → `set_mode(FAST)` → `begin_retire` (DRAINING→RETIRING) → **returns**. The close is an **Rx-lcore-owned observed retire**: the end-of-poll scan closes RETIRING→CLOSED itself once old-path quiescence holds (K empty Rx epochs + tail-idle measured from RETIRING entry) — the Comch thread never waits and a persistent late tail defers the close instead of leaking the slot. DROP/DELETE teardown is **fully Rx-lcore-owned**: `begin_close`, called immediately after the HW commit, sets a per-slot **discard flag** and returns (ACTIVE additionally moves to CLOSING, counted in `nr_closing`). The Rx lcore frees the backlog and any late in-flight DMA arrival, then closes the slot once the **observed-idle fence** holds — the same K-empty-epochs + tail-idle fence as the retire, floor-stamped by `begin_close`, pushed forward by every freed late arrival (`close_fence_holds`). The fence is the **reuse guard**: it replaces the old 50µs settle by keeping the slot out of CLOSED (hence out of `register_flow` reuse) until the previous cycle's residual NIC tail has gone quiet, so a fast same-`hw_rule_id` re-BUFF cannot capture an old session's packet into the new cycle. No quiesce, the Comch thread never blocks, and a deleted session's backlog never reaches the wire. Drained and late in-flight packets are SW GTP-U+PSC encapped (`dpu_gtp_encap_dl`) and Tx'd on **N3 q2**. UL BUFF is rejected (`DOCA_ERROR_NOT_SUPPORTED`). Full protocol: [DPU_ARM_BUFFERING_STRATEGY.md](../../DPU_ARM_BUFFERING_STRATEGY.md).
...
**Buffer slot lifecycle invariant** (BUFF→FORW→BUFF on the same `hw_rule_id`): no close path may **ever** `rte_hash_del_key` the flow binding — closes only set `CLOSED`. **Every close path publishes the `CLOSED` store-RELEASE LAST** (cleanup → counter releases → state): `register_flow`'s RETIRING grace spins on state and reuses the ring (`ring_flush` + thread-unsafe `rte_ring_reset`) the instant it observes CLOSED, so any write after the CLOSED store races the reuse. This is one uniform rule for all four close sites — including the control-thread ones where it isn't strictly required — so reuse-safety never depends on which thread closed the slot. The persistent `rte_ring` is reused on the next BUFF (CLOSED branch in `register_flow`); `register_flow` **refuses a still-RETIRING/CLOSING slot** after a bounded grace (`DPU_BUFFER_REGISTER_WAIT_US`, 5 ms — covers the Rx lcore's async closes, both behind the ~1 ms observed-idle fence; only CLOSED is reusable). All teardown closes are **Rx-lcore-owned** end-of-poll scans, each gated on its own counter and releasing exactly the counts its state holds: RETIRING→CLOSED decrements only `nr_retiring` (`begin_retire` already released `nr_draining`+`nr_buffering`); DRAINING→CLOSED (Phase 1 discard) releases `nr_draining`+`nr_buffering`; CLOSING→CLOSED releases `nr_closing`+`nr_buffering`. `dpu_buffer_rollback_register` (ACTIVE→CLOSED, register-first failure path, control thread) releases `nr_buffering` only. Bindings are **never** released at runtime — dropping one orphans the named ring and makes the next `rte_ring_create("buf_<id>")` fail (name still exists); the table is bounded by `max_flows` distinct buffered rules, which is **clamped to `RTE_MIN(max-hw-rules, 16384)`** at init (buffer+shaper capacity is bounded by `m-op-bytes`/`m-shape-bytes`, not rule count).
...
- **Buffer slot lifecycle is decoupled from pipeline `current_mode`.** After FORW, `set_mode(FAST)` runs immediately (HW is FAST once the override is gone); the SW slot may still be **RETIRING** while pass-through draining the late DMA tail. The RETIRING→CLOSED close happens on the **Rx lcore** at end-of-poll — *observed retire with bounded idle confidence, not deterministic*. The control thread never performs a data-path close: DROP/DELETE only sets `begin_close`'s discard flag and the Rx lcore frees + closes behind the observed-idle fence (the sole control-thread close is `rollback_register`, which runs before any packet can exist).
